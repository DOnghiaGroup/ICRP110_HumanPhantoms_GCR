...
flux_data_filename = "solar_max_flux.dat"
data_filenames = ["geant4_runs/doseDepositByPrimary_Basic_freespace_1e6runs.dat",
                  "geant4_runs/doseDepositByPrimary_Basic_Al_5gcm2_1e6runs.dat",
                  "geant4_runs/doseDepositByPrimary_Basic_Al_20gcm2_1e6runs.dat",
                  "geant4_runs/doseDepositByPrimary_Basic_Al_40gcm2_1e6runs.dat"]
...
# Make an array with a dataframe for each file
runs = []
...
for k in range(len(runs)):
    total_dose = 0.0
    bins.append(np.geomspace(xlims[0], xlims[1], numbins))
    bin_heights.append([0.0])

    for i, row in runs[k].iterrows():
        total_dose += (row['dose']*row['weight'])

    cumulative_dose = 0.0
    for i in range(len(bins[k])-1):
        for j, row in runs[k].iterrows():
            if (row['energy'] > bins[k][i]) and (row['energy'] < bins[k][i+1]):
                cumulative_dose += (row['dose']*row['weight'])
        bin_heights[k].append(cumulative_dose/total_dose)
...
for i in range(len(runs)):
    plt.plot(bins[i], bin_heights[i], label=labels[i], linewidth=3)

plt.xscale('log')
plt.xlabel('Kinetic Energy (MeV/n)', size=20)
plt.ylabel('Fraction of cumulative dose', size=20)
plt.xticks(size=20)
plt.yticks(size=20)
plt.grid(True)
plt.legend(fontsize=20)

plt.savefig("cumulative_dose_dist.jpg", dpi=1000)
